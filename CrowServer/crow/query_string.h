...
namespace crow 
{
    ...
    public:
        ...
        query_string(const query_string& qs)
            : url_(qs.url_)
        {
            for(auto p:qs.query_values)
            {
                query_values.push_back((char*)(p-qs.url_.c_str()+url_.c_str()));
            }
        }
        ...
        {
            query_values = std::move(qs.query_values);
            char* old_data = (char*)qs.url_.c_str();
            url_ = std::move(qs.url_);
            for(auto& p:query_values)
            {
            ...
            : url_(std::move(url))
        {
            ...
            query_values.resize(MAX_KEY_VALUE_PAIRS_COUNT);

            int count = qs_parse(&url_[0], &query_values[0], MAX_KEY_VALUE_PAIRS_COUNT);
            query_values.resize(count);
        }
        ...
        {
            os << "[ ";
            for(size_t i = 0; i < qs.query_values.size(); ++i) {
                if (i)
                    os << ", ";
                os << qs.query_values[i];
            }
            os << " ]";
            return os;
        ...
        {
            char* ret = qs_k2v(name.c_str(), query_values.data(), query_values.size());
            return ret;
        }
        ...
        std::vector<char*> get_list (const std::string& name) const
        {
            ...
            while(1)
            {
                element = qs_k2v(plus.c_str(), query_values.data(), query_values.size(), count++);
                if (!element)
                    break;
